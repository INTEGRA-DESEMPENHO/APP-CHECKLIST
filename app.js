...
// ─── Filtros / datalists ─────────────────────────────────
// ─── Multiselect (Bloco / Pavimento / Subambiente) ───────
// Estado: para cada campo, conjunto de valores marcados. Vazio = "todos".
var MS = { b:[], pav:[], sub:[] };
var MS_OPTS = { b:[], pav:[], sub:[] };

function getMulti(id){ return (MS[id]||[]).slice(); }      // valores marcados (array)
function setMulti(id,arr){ MS[id]=(arr||[]).slice(); }

function msLabel(id){
  var sel=MS[id]||[], total=(MS_OPTS[id]||[]).length;
  if(!sel.length) return 'Todos...';
  if(sel.length===total && total>0) return 'Todos ('+total+')';
  if(sel.length===1) return sel[0];
  return sel.length+' selecionados';
}
function msRender(id){
  var pop=document.getElementById('pop-'+id); if(!pop)return;
  var opts=MS_OPTS[id]||[], sel=MS[id]||[];
  var allChk=sel.length===opts.length && opts.length>0;
  var html='<label class="ms-all"><input type="checkbox" class="ms-cktodos" data-ms="'+id+'"'+(allChk?' checked':'')+'> <b>Selecionar tudo</b></label><div class="ms-list">';
  html+=opts.map(function(v){
    var on=sel.indexOf(v)>=0;
    return '<label class="ms-item"><input type="checkbox" class="ms-ck" data-ms="'+id+'" value="'+esc(v)+'"'+(on?' checked':'')+'> '+esc(v)+'</label>';
  }).join('');
  html+='</div>';
  pop.innerHTML=html;
  // atualiza rótulo do botão e estado "ok" (preenchido)
  var ms=document.getElementById('ms-'+id);
  if(ms){
    ms.querySelector('.ms-lbl').textContent=msLabel(id);
    ms.classList.toggle('ok', (MS[id]||[]).length>0);
  }
}
function msToggleOpen(id){
  var pop=document.getElementById('pop-'+id); if(!pop)return;
  var aberto=pop.classList.contains('open');
  document.querySelectorAll('.ms-pop.open').forEach(function(p){p.classList.remove('open');});
  if(!aberto)pop.classList.add('open');
}
// preenche as opções de cada multiselect a partir das linhas carregadas
function msPopular(){
  var bS={},pS={},sS={};
  ...
  MS_OPTS.b   = Object.keys(bS).sort();
  MS_OPTS.pav = Object.keys(pS).sort();
  MS_OPTS.sub = Object.keys(sS).sort();
  // remove de MS valores que não existem mais
  ['b','pav','sub'].forEach(function(id){
    MS[id]=(MS[id]||[]).filter(function(v){return MS_OPTS[id].indexOf(v)>=0;});
    msRender(id);
  });
}
...
function popularFiltros(unicas){
  ...
  if(ul)ul.innerHTML=(unicas||[]).map(opt).join('');
  msPopular();
  var sf=lsGet(LS_FILTROS)||{};
  document.getElementById('resp').value=sf.resp||'';
  document.getElementById('u').value   =sf.u   ||'';
  MS.b   = Array.isArray(sf.b)  ?sf.b.filter(function(v){return MS_OPTS.b.indexOf(v)>=0;})  :[];
  MS.pav = Array.isArray(sf.pav)?sf.pav.filter(function(v){return MS_OPTS.pav.indexOf(v)>=0;}):[];
  MS.sub = Array.isArray(sf.sub)?sf.sub.filter(function(v){return MS_OPTS.sub.indexOf(v)>=0;}):[];
  ['b','pav','sub'].forEach(msRender);
  document.getElementById('fst').value =sf.fst ||'NAO_AVALIADOS';
  ['resp','u'].forEach(mkpre);
}
function mkpre(id){
...
function getItens(){
  ...
  if(!uv)return[];
  var bv=getMulti('b').map(nrm), pv=getMulti('pav').map(nrm), sv=getMulti('sub').map(nrm);
  var ch=uv+'|'+bv.join(',')+'|'+pv.join(',')+'|'+sv.join(',');
  if(_cc===ch&&_ci)return _ci;
  _cc=ch;
  _ci=ROWS.filter(function(item){
    var ui=(item[DI.UNINORM]||'').trim();
    return ui===uv&&
      (bv.length===0||bv.indexOf(nrm(item[DI.BLC]))>=0)&&
      (pv.length===0||pv.indexOf(nrm(item[DI.PAV]))>=0)&&
      (sv.length===0||sv.indexOf(nrm(item[DI.SUB]))>=0);
  });
  return _ci;
}
...
function renderLista(){
  ...
  var subSel=getMulti('sub');
  var sub=subSel.length===1?subSel[0]:''; // 1 selecionado = abre cards; vários/nenhum = lista
...
function limparSecundarios(){
  setMulti('b',[]);setMulti('pav',[]);setMulti('sub',[]);
  ['b','pav','sub'].forEach(msRender);
  savFilt();invCache();invRsCache();renderLista();updCDebounced();
}
// Limpa um único filtro (botão "x" da unidade).
function limparUm(id){
  if(id==='b'||id==='pav'||id==='sub'){
    setMulti(id,[]);msRender(id);savFilt();invCache();invRsCache();renderLista();updCDebounced();return;
  }
  var el=document.getElementById(id);
  if(!el)return;
  el.value='';
...
function updPBar(){
  var ss=getMulti('sub'); var sub=ss.length===1?ss[0]:''; if(!sub)return;
...
async function tentarSalvar(){
  ...
  var u=document.getElementById('u').value.trim();
  if(!resp||!u){toast('Preencha Inspetor e Unidade.',4000);return;}
  ...
  // agrupa os itens por bloco (a sessão pode ter vários blocos selecionados)
  var porBloco={};
  its.forEach(function(s){
    var bl=s.bloco||'';
    if(!porBloco[bl])porBloco[bl]=[];
    porBloco[bl].push(s);
  });
  var blocos=Object.keys(porBloco);
  if(!blocos.length)blocos=['']; // só fotos de subambiente, sem itens

  var pacotes=blocos.map(function(bl,idx){
    return {
      r:resp,u:u,b:bl,sub:'',
      fotosSubambiente: idx===0?FSUB.map(function(f){return f.b64;}):[],
      itens:(porBloco[bl]||[]).map(function(s){return{
  ...
  try{
    var fila=getFilaSave().concat(pacotes);
...
function irParaSub(uni,bl,pav,sub,st){
  ...
  setMulti('b',  bl ?[bl] :[]);
  setMulti('pav',pav?[pav]:[]);
  setMulti('sub',sub?[sub]:[]);
  ['b','pav','sub'].forEach(msRender);
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  // Unidade (única) — dispara recarga
  var uEl=document.getElementById('u');
  if(uEl)uEl.addEventListener('change',function(){
    savFilt();invCache();invRsCache();carregar();
  });

  // Multiselect: abrir/fechar ao clicar no botão
  document.querySelectorAll('.ms-btn').forEach(function(btn){
    btn.addEventListener('click',function(e){ e.stopPropagation(); msToggleOpen(btn.dataset.ms); });
  });
  // Marcar/desmarcar opções e "Selecionar tudo"
  document.addEventListener('change',function(e){
    var ck=e.target;
    if(ck.classList&&ck.classList.contains('ms-ck')){
      var id=ck.dataset.ms, v=ck.value, arr=getMulti(id);
      var i=arr.indexOf(v);
      if(ck.checked){ if(i<0)arr.push(v); } else { if(i>=0)arr.splice(i,1); }
      setMulti(id,arr);msRender(id);
      savFilt();invCache();invRsCache();renderLista();updCDebounced();
    }else if(ck.classList&&ck.classList.contains('ms-cktodos')){
      var id2=ck.dataset.ms;
      setMulti(id2, ck.checked?(MS_OPTS[id2]||[]).slice():[]);
      msRender(id2);
      savFilt();invCache();invRsCache();renderLista();updCDebounced();
    }
  });
  ...
  document.addEventListener('click',function(e){
    if(!e.target.closest||!e.target.closest('.ms')){
      document.querySelectorAll('.ms-pop.open').forEach(function(p){p.classList.remove('open');});
    }
  });
